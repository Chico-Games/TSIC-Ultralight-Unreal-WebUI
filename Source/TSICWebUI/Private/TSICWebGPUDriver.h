...
namespace TSICWebUI
{
	...
	private:
		...
		// Render-thread-owned. Each driver callback (CreateTexture, UpdateTexture,
		// CreateGeometry, etc.) enqueues its own render command which mutates these
		// maps on the render thread. All those commands run sequentially on the
		// render thread, so map access is serialised without explicit locking. The
		// draw-loop render command (ExecuteCommands) reads them, also on the render
		// thread, after all prior commands have run. The destructor flushes
		// rendering commands before clearing these on the game thread.
		...
		// Game-thread-readable mirror of texture RHI refs. Written by the render
		// thread after texture creation, and by the game thread for external
		// textures (RegisterExternalTexture / Unregister). Locked because the two
		// threads can write concurrently.
